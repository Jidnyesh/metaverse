...
public class NetworkDebugStart : Fusion.Behaviour
{
    ...
    {
        if (CurrentStage == Stage.StartingUp || AutoStartAs == GameMode.Client)
        {
            Debug.Log("loading");
            loading.SetActive(true);
        }
        if (CurrentStage == Stage.AllConnected)
        {
    ...
    protected virtual void Start()
    {
        ...
        int c1 = PlayerPrefs.GetInt("count");
        string check = PlayerPrefs.GetString("CHAR");
        if (check == "Teacher")
        {
            StartMode = StartModes.Automatic;
            AutoStartAs = GameMode.Host;
            PlayerPrefs.SetString("CHAR", "NONE");
        }
        else
        if (check == "Student")
        {
        ...
        if (AutoStartAs == GameMode.Client)
        {
            Debug.Log("Hand");
            Hand.SetActive(true);
        }
        if (StartMode == StartModes.Automatic && AutoStartAs == GameMode.Single)
        {
    ...
    public void detectTeacher(string room)
    {
        DefaultRoomName = room;
        count = 1;
        string character = "Teacher";
    ...
    public void detectStudent(string room)
    {
        DefaultRoomName = room;
        count = 1;
        string character = "Student";
        PlayerPrefs.SetString("CHAR", character);
        PlayerPrefs.SetInt("number", 0);
        Shutdown();
        //Debug.Log(id);
